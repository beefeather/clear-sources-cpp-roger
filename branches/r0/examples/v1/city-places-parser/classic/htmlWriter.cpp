...
HTMLWriter::HTMLWriter(vector<Place>& places):
	places_(places) { }	

HTMLWriter::~HTMLWriter() { }

void HTMLWriter::Save(string path){
	string TITLE = "Title";	
	...
	for(std::vector<Place>::iterator i = places_.begin(); i != places_.end(); ++i){
		...
		fout << "\t\t<tr>\n";	
		fout << "\t\t\t<td>" << (*i).getName() << " (" << commentNum << ")" << "</td>\n";
		fout << "\t\t\t<td>" << (*i).getAddress() << "</td>\n";
		fout << "\t\t\t<td> <a href=\"" << (*i).getComment()->getURL() << "#t" << (*i).getComment()->getID() << "\">Комментарий</a></td>\n";		
		fout << "\t\t\t<td><span style=\"color:green\">+" << (*i).getPositiveRank() << 
					"</span> <span style=\"color:red\">-" << (*i).getNegativeRank() << "</td>\n";
		fout << "\t\t</tr>\n";	
