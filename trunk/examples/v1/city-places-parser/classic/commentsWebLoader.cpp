...
void CommentsWebLoader::LoadComments(string url, LjPost* rootComment, string filePathPrefix) {	
	url_ = url;	
...
void CommentsWebLoader::LoadPagesFromWeb() {
	...
	do{
		...
		if(countBeginPos == -1) break;

		countBeginPos += 12;
		countEndPos = page.find("]",countBeginPos);	
		if (countEndPos == -1) break;

		count = atoi( page.substr(countBeginPos, countEndPos - countBeginPos).c_str() );
	}while(true);	
	pages_ = count;
	cout << "pages: " << count << "\n";
...
void CommentsWebLoader::LoadCommentsFromWeb() {	
	for(int i = 1; i <= pages_; ++i)
	{
		...
		do{		
			...
			string commentID = page.substr(commentIdBeginPos, commentIdEndPos - commentIdBeginPos);
			...
			int commentWidthBeginPos = page.find(commentWidthBeginTag, commentIdEndPos) + commentWidthBeginTag.length(); 
			int commentWidthEndPos = page.find("'", commentWidthBeginPos);
			int commentWidth = atoi( page.substr(commentWidthBeginPos, commentWidthEndPos - commentWidthBeginPos).c_str() );
			...
			string thread;
			string threadRequest = "http://tema.livejournal.com/tema/__rpc_get_thread?journal=tema&itemid=" + postNum_ + "&thread=" + commentID;
			Network network;
...
void CommentsWebLoader::addCommentFromWeb(Container* parent, string thread){
	...
	do{
		...
		if(commentIdBeginPos == -1) return; //no comments
		commentIdBeginPos += commentIDBeginTag.length();
		int commentIdEndPos = thread.find("'", commentIdBeginPos);
		string str_currCommentID = thread.substr(commentIdBeginPos, commentIdEndPos - commentIdBeginPos);
		...
		int commentBeginPos = thread.find(commentBeginTag, commentIdEndPos) + commentBeginTag.length();
		int commentEndPos = thread.find(commentEndTag, commentBeginPos);
		string currCommentText = thread.substr(commentBeginPos, commentEndPos - commentBeginPos);
	
		//current comment parent id 
		string replyBeginTag = "?replyto";		
		int replyTagBeginPos = thread.find(replyBeginTag, commentEndPos) + replyBeginTag.length();	
		int parentIDBeginPos = thread.find("#t",replyTagBeginPos) + 2;	
		parentIDEndPos = thread.find("'",parentIDBeginPos);
		int parentID = atoi( thread.substr(parentIDBeginPos, parentIDEndPos - parentIDBeginPos).c_str() );
...
string CommentsWebLoader::intToStr(int number){
